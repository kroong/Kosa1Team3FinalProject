				...
				.slider(
						{
							...
							slide : function(event, ui) {
								...
								var email = document.getElementById("myTable").rows[row].cells
										.namedItem("email").innerHTML;
								...
								if (ui.value == 1) {
									...
									var blocked = document
											.getElementById("myTable").rows[row].cells
											.namedItem("blocked").innerHTML;
									console.log("blocked:" + blocked);

									if (blocked.indexOf("차단된친구") > -1) {
										$("input[name=amount2]").eq(row - 1)
												.val("차단");
						...
						selectList
								.forEach(function(select) {
									var optionList = select
											.querySelectorAll('.option'), selectedIndex = getIndex(select);
									...
									select
											.addEventListener(
													'keyup',
													function(event) {
														var length = optionList.length, index = getIndex(select);
...
#large-header {
	...
	display: block;
	background-image:
		url('${pageContext.request.contextPath}/resources/image/menubg.PNG');
	background-repeat: no-repeat;
	background-size: cover;
	height: 100px;
	padding: 5px;
...
<!-- -------2. 내정보----------------------------------------------------------------------------------------------------- -->
				...
				<div id="albumitlogo">
					<img
						...
						alt="Albumit" title="Albumit" id="logo">
				</div>
				<!-- -------------------------------------------------------------------------------------------------------------------- -->
	...
	<!-- --------4. 검색------------------------------------------------------------------------------------------------------- -->
				<!-- <div class="input-group margin-bottom-sm menu"> -->
					...
					<div id="search" class="input-group margin-bottom-sm menu">
						...
						<br />
						<p id="validation">
							<em title="Please enter something you want to search"
								class="validation"></em>
						</p>
			...
			<div id="tabs">
				...
				<div id="mypage">
					...
					<img
						...
						width="100px" height="100px" /><br /> <a href="#">pw ?</a> <a
						href="leave">leave Albumit</a>
			...
			<form method="post" action="makeAlbum">
				<fieldset id="addAlbumField">
					<label for="album_name">Album Name</label> <input type="text"
						id="album_name" name="album_name" size="20" /><br /> <input
						type="hidden" name="uid" value="${member.uid }" />
					<hr />
					<p>Would you like to open this album to public?</p>
					<input type="radio" name="album_publicity" value="true" />yes <input
						type="radio" name="album_publicity" value="false" />no<br />
					<hr />
